...
## Build fix
- Fixed iOS 16 navigation by replacing `navigationDestination(item:)` with direct `NavigationLink`.
- Added the missing Liquid Glass `GlassToggle` component.
- Simplified the Patch Project detail footer expression for Swift compiler type-checking.
- Kept `PatchProjectDetailView` accessible from the main content view.
