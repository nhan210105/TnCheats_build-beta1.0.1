...
struct PatchProjectsView: View {
    ...
    private func consumeExternalImport() {
        guard developerModeEnabled, let request = draftCoordinator.importRequest else { return }
...
private struct PatchRuleRow: View {
    let rule: PatchRule
...
struct PatchProjectDetailView: View {
    ...
    private var applyConfirmationMessage: String {
        language.text("patch.apply_confirm_message")
    }

    private var restoreConfirmationMessage: String {
        language.text("patch.restore_confirm_message")
    }
    ...
    private func workspaceBundleIDRow(_ bundleID: String) -> some View {
        HStack(spacing: 10) {
    ...
    private func updateRule(_ updatedRule: PatchRule) {
        guard developerModeEnabled, var project = item?.project,
