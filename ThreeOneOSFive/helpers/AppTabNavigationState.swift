...
/// Controls access to owner/developer-only project editing features.
/// The plaintext key is never stored in the app; only its SHA-256 digest is embedded.
enum DeveloperAccessController {
    static let storageKey = FeatureVisibility.developerModeStorageKey
    static let expectedSHA256 = "10e447e2ad0f99b50a4e9e86fa66bf3579efd440bdb86ae28d9cb65256921333"

    static func verify(_ key: String) -> Bool {
        let normalized = key.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !normalized.isEmpty else { return false }
        let digest = SHA256.hash(data: Data(normalized.utf8))
        return digest.map { String(format: "%02x", $0) }.joined() == expectedSHA256
    }
